...
public class KickoffView extends Composite {
	...
	private void objectiveRiskControlMatrix(final AuditEngagement record, AccordionLayoutContainer con,
		...
		/*btnSaveControl.addClickHandler(new ClickHandler(){
			...
			public void onClick(ClickEvent event) {
				...
				for(int i=0; i< userRiskControlContainer.getWidgetCount() ; i++){
					RiskControlMatrixView existingControlView =  (RiskControlMatrixView)userRiskControlContainer.getWidget(i);
	...
	private void objectivesLayout(final AuditEngagement record, AccordionLayoutContainer con,
		...
		if(record.getEngagementDTO().getSelectedActivityObjectives().size()<=0 || record.getEngagementDTO().getSelectedActivityObjectives().get(0).getStatus() == InternalAuditConstants.SAVED){
