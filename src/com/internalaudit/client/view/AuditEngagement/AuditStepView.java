...
//				disableFields(exceptions);
		...
		reject.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent arg0) {
				...
				amendmentPopup.getBtnSubmit().addClickHandler(new ClickHandler() {
					...
					public void onClick(ClickEvent event) {
						disableFields(exceptions);
						saveAuditStep(auditWork, selectedJobId, InternalAuditConstants.REJECTED, amendmentPopup.getComments().getText());
	...
	private void saveAuditStep(final AuditWork auditWork,
			final int selectedJobId, int status, String feedback) {
//		disableFields(exceptions);
