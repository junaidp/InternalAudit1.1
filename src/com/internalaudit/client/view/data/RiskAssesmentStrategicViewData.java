...
//
				...
				if(caught instanceof TimeOutException){
					...
					Window.alert("FAIL: fetchRiskFactors");// After FAIL ... write RPC Name  NOT Method Name..
				}
			}
		...
		rpcService.fetchRiskAssesment(hm, new AsyncCallback<ArrayList<RiskAssesmentDTO>>(){
			...
			public void onFailure(Throwable caught) {
				...
				if(caught instanceof TimeOutException){
					...
					Window.alert("FAIL: fetchRiskAssesment");// After FAIL ... write RPC Name  NOT Method Name..
				}
...
//					strategicPanel.add(treeMain);
					...
					for(int j=0; j< riskFactors.size(); j++){
						...
						if(riskAssesmentDTOs.get(index).getStrategic().getPhase()!= 2 || riskAssesmentDTOs.get(index).getStrategic().getLoggedInUser()!= riskAssesmentDTOs.get(index).getStrategic().getAssignedTo().getEmployeeId()){
							disablePanel(riskAssesmentStrategicView, riskFactorsView, riskAssesmentDTOs.get(index).getStrategic());
						}else{
...
//		riskAssesmentDTOs.get(riskAssesmentStrategicView.getIndex()).getStrategic().setNextPhase("Consolidation");
	...
	public void disablePanel(RiskAssesmentStrategicView riskAssesmentStrategicView, RiskFactorsView riskFactorsView, Strategic strategic){
		...
		riskAssesmentStrategicView.getSubmitted().setTitle(strategic.getStatus() + ": In "+ PhaseNames.getPhaseNames(strategic.getPhase()));
		...
		rpcService.saveRiskAssesment(hm, strategicRisks, new AsyncCallback<String>(){
			...
			public void onFailure(Throwable caught) {
				...
				if(caught instanceof TimeOutException){
					...
					Window.alert("FAIL: saveRiskAssesment");// After FAIL ... write RPC Name  NOT Method Name..
				}
		...
		rpcService.sendBackStrategic(strategicToSendBack, new AsyncCallback<String>(){
			...
			public void onFailure(Throwable caught) {
				...
				if(caught instanceof TimeOutException){
					...
					Window.alert("FAIL: sendBackStrategic");// After FAIL ... write RPC Name  NOT Method Name..
				}
			}
