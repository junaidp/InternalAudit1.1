		...
		rpcService.checkDate(strategicView.getObjectiveAchievementDate().getValue(), new AsyncCallback<Boolean>(){
			...
			public void onFailure(Throwable caught) {
				...
				if(caught instanceof TimeOutException){
					...
					Window.alert("FAIL: checkDate");// After FAIL ... write RPC Name  NOT Method Name..
				}
...
//		strategics.add(strategic);
	...
	public void fetchObjectiveOwners(){
		...
		rpcService.fetchObjectiveOwners(new AsyncCallback<ArrayList<Employee>>(){
			...
			public void onFailure(Throwable caught) {
				/////These are the lines we need to paste in every method's onFailure
				///From here
				logger.log(Level.INFO, "FAIL: fetchObjectiveOwners .Inside Audit UniverseStrategicViewData");
				// please change the class name , method's name accordingly in other methods//
				if(caught instanceof TimeOutException){
					History.newItem("login");
				}else{
...
//
	...
	public void fetchStrategic(final VerticalPanel vpnlStrategic, 
			final HorizontalPanel hpnlButtonInitiator, final HorizontalPanel hpnlButtonsApprovar, final Image btnAdd, int tab){
		final LoadingPopup loadingPopup = new LoadingPopup();
		loadingPopup.display();
		HashMap<String,String> hm = new HashMap<String,String>();
	      hm.put("phase", "1");
...
//				btnAdd.setEnabled(true);
	...
	public void disablePanel(AuditUniverseStrategicView auditUniverseStrategicView, Strategic strategic){
		...
		auditUniverseStrategicView.getSubmitted().setTitle(strategic.getStatus() + ": In "+ PhaseNames.getPhaseNames(strategic.getPhase()));
...
//			} 
	...
	private void setHandlers(final VerticalPanel vpnlStrategic,
			...
			final Image btnAdd, final AuditUniverseStrategicView auditUniverseStrategicView, final int tab) {
