...
public class ConsolidationViewData {
	...
	public void saveConsolidation(Strategic strategic, final VerticalPanel vpnlData, ConsolidationView consolidationView, String todo, int tab, final Button buttonClicked){
		...
		String a = consolidationView.getListBoxProcess().getValue(consolidationView.getListBoxProcess().getSelectedIndex());
		process.setProcessId(Integer.parseInt(consolidationView.getListBoxProcess().getValue(consolidationView.getListBoxProcess().getSelectedIndex())));
		strategic.setProcess(process);

		SubProcess subprocess = new SubProcess();
		String s = consolidationView.getListBoxSubProcess().getValue(consolidationView.getListBoxSubProcess().getSelectedIndex());
		subprocess.setSubProcessId(Integer.parseInt(consolidationView.getListBoxSubProcess().getValue(consolidationView.getListBoxSubProcess().getSelectedIndex())));
		strategic.setSubProcess(subprocess);

		JobType jobtype = new JobType();
		jobtype.setJobTypeId(Integer.parseInt(consolidationView.getListBoxJobType().getValue(consolidationView.getListBoxJobType().getSelectedIndex())));
			...
			public void onSuccess(final ArrayList<Strategic> strategics) {
				...
				for(index=0; index<strategics.size(); index++){
					...
					for ( int i = 0;i<processDTO.get(0).getProcessList().size(); i++){
						consolidationView.getListBoxProcess().addItem(processDTO.get(0).getProcessList().get(i).getProcessName(), processDTO.get(0).getProcessList().get(i).getProcessId()+"");
					}

					for ( int i = 0;i<processDTO.get(0).getJobTypeList().size(); i++){
						consolidationView.getListBoxJobType().addItem(processDTO.get(0).getJobTypeList().get(i).getJobTypeName(), processDTO.get(0).getJobTypeList().get(i).getJobTypeId()+"");
					}

					for ( int i = 0; i<subProcess.size(); i++){
						consolidationView.getListBoxSubProcess().addItem(subProcess.get(i).getSubProcessName(), subProcess.get(i).getSubProcessId()+"");
					}

					//setting process subprocess and jobtype after populating them from above
					if(strategics.get(consolidationView.getIndex()).getProcess()!=null ){
						for(int i=0; i<consolidationView.getListBoxProcess().getItemCount(); i++){
							if( Integer.parseInt(consolidationView.getListBoxProcess().getValue(i)) == strategics.get(consolidationView.getIndex()).getProcess().getProcessId()){
								consolidationView.getListBoxProcess().setSelectedIndex(i);
								fetchSubProcess(Integer.parseInt(consolidationView.getListBoxProcess().getValue(i)), consolidationView.getListBoxSubProcess(), strategics.get(consolidationView.getIndex()).getSubProcess().getSubProcessId());
								break;
							}
						}
					...
					for(int i=0; i<consolidationView.getListBoxJobType().getItemCount(); i++){
						if(consolidationView.getListBoxJobType().getValue(i).equals(strategics.get(consolidationView.getIndex()).getJobType().getJobTypeId()+"")){
					...
					consolidationView.getListBoxProcess().addChangeHandler(new ChangeHandler() {
						...
						public void onChange(ChangeEvent event) {
							//Window.alert("changing:" + consolidationView.getListBoxProcess().getValue(consolidationView.getListBoxProcess().getSelectedIndex()));
							fetchSubProcess(Integer.parseInt(consolidationView.getListBoxProcess().getValue(consolidationView.getListBoxProcess().getSelectedIndex())), consolidationView.getListBoxSubProcess(), 0);
						}
					});
