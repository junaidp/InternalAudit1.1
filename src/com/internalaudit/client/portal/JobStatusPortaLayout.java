...
public class JobStatusPortaLayout extends VerticalLayoutContainer {
	...
	public JobStatusPortaLayout(JobStatusDTO jobStatus) {
		...
		portletPlanning.setHeadingHtml("Planning");
	...
	private void setData(JobStatusDTO jobStatus) {
		...
		for (int i = 0; i < jobStatus.getListFieldWorkStatus().size(); i++) {
			...
			// fieldWork.setUrl(jobStatus.getListFieldWorkStatus().get(i).isHaveExceptions()?
			// "redCircleNew.png": "greenCircleNew.png");
			fieldWork.setUrl(
					jobStatus.getListFieldWorkStatus().get(i).getStatus().equals(InternalAuditConstants.COMPLETED)
							? "greenCircleNew.png" : "redCircleNew.png");
		...
		for (int i = 0; i < jobStatus.getListPlanningStatus().size(); i++) {
			...
			planning.setUrl(planning.getStatus().equals(InternalAuditConstants.COMPLETED) ? "greenCircleNew.png"
					: "redCircleNew.png");
		...
		reporting0.setName(
				"Draft Report Approval By Head of Inernal Audit (" + InternalAuditConstants.EXCEPTIONSTOSENT + ")");
		reporting0.setId(0);
	...
	private void updateReportingStatusForOthers(String status) {
		...
		// listReporting.get(index).setUrl(listReporting.get(index).getStatus().equals(InternalAuditConstants.COMPLETED)?
		// "greenCircleNew.png" : "redCircleNew.png");

		listReporting.get(index).setUrl("greenCircleNew.png");

		/*
		 * for(int i=0; i< listReporting.size(); i++){ if(!
		 * status.equals(InternalAuditConstants.COMPLETED) && i> index){
		 * listReporting.get(i).setStatus(InternalAuditConstants.NOT_STARTED);
		 * listReporting.get(i).setUrl(listReporting.get(i).getStatus().equals(
		 * InternalAuditConstants.COMPLETED)? "greenCircleNew.png" :
		 * "redCircleNew.png");
		 * 
		 * }else if(status.equals(InternalAuditConstants.COMPLETED) && i<
		 * index){
	...
	public Widget createGridFieldWork() {
		...
		ColumnConfig<FieldWork, String> fieldworkCol = new ColumnConfig<FieldWork, String>(fieldWorkProperties.name(),
				150, "Name");
		// ColumnConfig<FieldWork, Date> lastTransCol = new
		// ColumnConfig<FieldWork, Date>(fieldWorkProperties.date(), 100,
		// "Date");
		ColumnConfig<FieldWork, String> statusCol = new ColumnConfig<FieldWork, String>(fieldWorkProperties.status(),
				150, "Status");
	...
	public Widget createGridPlanning() {
		...
		ColumnConfig<Planning, String> nameCol = new ColumnConfig<Planning, String>(planningProperties.name(), 150,
				"Name");
		// ColumnConfig<Planning, Date> dateCol = new ColumnConfig<Planning,
		// Date>(planningProperties.date(), 100, "Date");
		ColumnConfig<Planning, String> statusCol = new ColumnConfig<Planning, String>(planningProperties.status(), 150,
				"Status");
	...
	public Widget createGridReporting() {
		...
		ColumnConfig<Reporting, String> nameCol = new ColumnConfig<Reporting, String>(reportingProperties.name(), 150,
				"Name");
		// ColumnConfig<Reporting, Date> dateCol = new ColumnConfig<Reporting,
		// Date>(reportingProperties.date(), 100, "Date");
		ColumnConfig<Reporting, String> statusCol = new ColumnConfig<Reporting, String>(reportingProperties.status(),
				150, "Status");
