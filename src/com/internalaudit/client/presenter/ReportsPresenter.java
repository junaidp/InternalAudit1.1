			...
			public void onSuccess(ArrayList<Employee> arg0) {
				...
				for ( Employee e : arg0){
					...
					display.getReport5().getLstResponsibleAuditee().addItem( e.getEmployeeName(), String.valueOf(e.getEmployeeId()));
						...
						display.getReport4().getLstResource().addItem( e.getEmployeeName(), String.valueOf(e.getEmployeeId()));
...
//				final ReportAuditScheduling view = new ReportAuditScheduling(null);
				
				rpcService.fetchReportAuditScheduling(div, domain, jobStatus,selectedEmp, new AsyncCallback<ArrayList<Strategic>>() {
...
//								resultsTable.getRowFormatter().addStyleName(i+1, "form-row");
								...
								resultsTable.setWidget(i+1, j++, new Label( strategicList.get(i).getJobName() ));
...
//								resultsTable.setWidget(i+1, j++, new Label( strategicList.get(i).getAssignedTo().getEmployeeName() ));
								...
								resultsTable.setWidget(i+1, j++, new Label( strategicList.get(i).getEstimatedWeeks()+" weeks"));
							}
...
//				final ReportJobTimeAllocation view = new ReportJobTimeAllocation(null);
				
				rpcService.fetchReportWithResourcesSearchResult(div, domain, risk,resources, new AsyncCallback<ArrayList<JobCreation>>() {
...
//								resultsTable.setWidget(i+1, j++, new Label( strategicList.get(i).getStrategicObjective() ));
								...
								resultsTable.setWidget(i+1, j++, new Label(jobsList.get(i).getEstimatedWeeks() + "("+ jobsList.get(i).getStartDate()+ " - " +jobsList.get(i).getEndDate() +")"));
//								resultsTable.setWidget(i+1, j++, new Label( jobsList.get(i).getDivisionName()));
//								resultsTable.setWidget(i+1, j++, new Label( jobsList.get(i).getRating() ));
...
//							chartHpnl.add( createChart(jobsList, false));
//							
//							chartHpnl.add( createDomainChart(jobsList, false));
							chartHpnl.setHorizontalAlignment(HasHorizontalAlignment.ALIGN_CENTER);
							
//							chartHpnl.add( createDivisionChart(ids));
//							
//							createDivisionChart(ids, false, view); ////////////////////////Commented this///
//							
//							display.getReport4().getTablePanel().add( chartHpnl);
							
							display.getReport4().getTablePanel().add( createJobTimeAllocationChart(jobsList));
//							
//							display.getReport1().getDetailedChartsView().add( createDomainChart(jobsList, true));
							
//							createDivisionChart(ids, true, display.getReport4());
...
//				final ReportJobTimeAllocation view = new ReportJobTimeAllocation(null);
				...
				rpcService.fetchExceptionReports(div, domain, risk,resources,jobs,auditees,exceptionStatus, new AsyncCallback<ArrayList<Exceptions>>() {
...
//							resultsTable.setWidget(0, 2, new Label("Division"));
//							resultsTable.setWidget(0, 3, new Label("Domain"));
							resultsTable.setWidget(0, 2, new Label("Status"));
//							resultsTable.setWidget(0, 5, new Label("Resource"));
							resultsTable.setWidget(0, 3, new Label("Auditee"));
//							resultsTable.setWidget(0, 7, new Label("Risk"));
...
//							resultsTable.getCellFormatter().addStyleName(0, 4, "labelHeading");
//							resultsTable.getCellFormatter().addStyleName(0, 5, "labelHeading");
//							resultsTable.getCellFormatter().addStyleName(0, 6, "labelHeading");
//							resultsTable.getCellFormatter().addStyleName(0, 7, "labelHeading");
...
//							resultsTable.getCellFormatter().addStyleName(0, 4, "labelHeading");
							...
							for ( int i = 0; i < exceptionsList.size(); i++ )
							{
								...
								resultsTable.setWidget(i+1, j++, new Label( exceptionsList.get(i).getDetail()));
								resultsTable.setWidget(i+1, j++, new Label(exceptionsList.get(i).getJobName()));
								resultsTable.setWidget(i+1, j++, new Label( exceptionsList.get(i).getDisplayStatus()));
								resultsTable.setWidget(i+1, j++, new Label( exceptionsList.get(i).getResponsiblePerson().getEmployeeName()));
								resultsTable.getRowFormatter().addStyleName(i, "form-row");
								resultsTable.getCellFormatter().addStyleName(i+1, j++, "form-row");
							}
...
//        if ( !isDetailed ) chart.setSize(350, 300); else chart.setSize(820, 500);
        if ( !isDetailed ) chart.setSize(report1RiskChartWidth, report1RiskChartHeight); else chart.setSize(820, 500);
	...
	public Chart createJobTimeAllocationChart(ArrayList<JobCreation> jobs) {  
          ...
          .setType(Series.Type.PIE)  
          .setChartTitleText("Job Time Allocation")  
          .setPlotBackgroundColor((String) null)  
          .setPlotBorderWidth(null)  
          .setPlotShadow(false)  ;
          chart.setCredits(
...
//          })  
	...
	public Chart createExceptionsChart(ArrayList<Exceptions> exceptions) {  
		  ...
		  final Chart chart = new Chart()  
        .setType(Series.Type.PIE)  
        .setChartTitleText("Exceptions Staus")  
        .setPlotBackgroundColor((String) null)  
        .setPlotBorderWidth(null)  
        .setPlotShadow(false) ;
        chart.setCredits(
...
//            .setCursor(PlotOptions.Cursor.POINTER)  
	...
	public int getExceptionStatusCount(String exceptionStatus, ArrayList<Exceptions> exceptions){
		int count =0;
		for(int i=0; i< exceptions.size(); i++){
			if(exceptions.get(i).getDisplayStatus()!=null && exceptions.get(i).getDisplayStatus().equals(exceptionStatus)){
				count++;
			}
		}
...
//        if ( !isDetailed ) chart.setSize(380, 300); else chart.setSize(820, 500);
        if ( !isDetailed ) chart.setSize(report1DomainChartWidth, report1DomainChartHeight); else chart.setSize(820, 500);
        return chart;  
    }
...
//		        	   chart.setSize(480, 300); 
		        	   if ( !isDetailed ) chart.setSize(report1DivsionChartWidth, report1DivisionChartHeight); else chart.setSize(820, 500);
		        	   chartHpnl.add( chart ); }
