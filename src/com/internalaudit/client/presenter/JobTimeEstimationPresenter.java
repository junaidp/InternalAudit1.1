...

{
	...
	private void bind() {
		...
		rpcService.fetchJobTime(display.getStrategicDTO().getStrategicId(), new AsyncCallback<JobTimeEstimationDTO>() {
			...
			public void onFailure(Throwable caught) {
				...
				if(caught instanceof TimeOutException){
					...
					Window.alert("FAIL: fetchJobTime");// After FAIL ... write RPC Name  NOT Method Name..
				}
		...
		display.getHeading().setText( display.getStrategicDTO().getAuditableUnit() );// to unit
				...
				try {
					...
					{		
						...
						if ( hoursReqd > res.getAvailableHours() )
						{
							...
							Window.alert("You have selected more hours than available hours for " + res.getSkillsList().getItemText(res.getSkillsList().getSelectedIndex()));
							//return;
				...
				rpcService.saveJobTimeEstimation(dto , updateForSkills, new AsyncCallback<Boolean>() {
					...
					public void onFailure(Throwable caught) {
						...
						if(caught instanceof TimeOutException){
							...
							Window.alert("FAIL: saveJobTimeEstimation");// After FAIL ... write RPC Name  NOT Method Name..
						}
			...
			public void onSuccess(ArrayList<Skills> skillsList) {	
				...
				for( int i = 0; i < skillsList.size(); i++)
				{
					...
					if(skillsList.get(i).getCompanySkillRelation() == null){
						...
						skillsResources.setAvailableHours( skillsList.get(i).getCompanySkillRelation().getAvailableHours());
					}
					...
					skillsResources.getNoOfResources().addBlurHandler(new BlurHandler() {
						...
						public void onBlur(BlurEvent event) {
							int skillId = Integer.parseInt(skillsResources.getSkillsList().getValue(skillsResources.getSkillsList().getSelectedIndex()));
							try{
...
//							fetch
				...
				int estimatedWeeks = Integer.parseInt(display.getEstWeeksListBox().getItemText( display.getEstWeeksListBox().getSelectedIndex()));
				int listSize = /*((SkillsResources)*/display.getSkillsResourceContainer().getWidgetCount();//(0)).getSkillsList().getItemCount();
