		...
		rpcService.fetchCreatedJob( strategicDTO.getStrategicId(), true, true, "jobId", new AsyncCallback<JobCreationDTO>() {
			...
			public void onSuccess(JobCreationDTO jobCreationDTO) {
				if ( jobCreationDTO == null )
				{
					...
					System.out.println(" Job creation found with id : "  + jobCreationDTO.getJob().getJobCreationId());
...
//				dto.getJob().setJobName( display.getStrategicDTO().getStrategicObjective());
				dto.getJob().setJobName( display.getStrategicDTO().getAuditableUnit());// to unit
				...
				for ( int i = 0; i < display.getProposedResources().getItemCount(); ++i)
				{
					if ( display.getProposedResources().isItemSelected(i))
					{
						...
						try{
							r.setEmployee(e);
//							r.setId( Integer.parseInt(display.getProposedResources().getValue(i).split("-")[0] ));
						}catch(Exception ex){
				...
				if(display.getAuditHead().getSelectedIndex()!=0){
					...
					head.setEmployeeId( Integer.parseInt(display.getAuditHead().getValue(display.getAuditHead().getSelectedIndex()
							)));
...
//						r.setId( Integer.parseInt(display.getProposedResources().getValue(i).split("-")[0] ));
				...
				rpcService.saveCreatedJob(dto, new AsyncCallback<Void>() {
					...
					public void onFailure(Throwable caught) {
						...
						if(caught instanceof TimeOutException){
							...
							Window.alert("FAIL: saveCreatedJob");// After FAIL ... write RPC Name  NOT Method Name..
						}
		...
		rpcService.fetchResourceUseFor(dto.getStrategicId(), new AsyncCallback<ArrayList<ResourceUse>>() {
			...
			public void onFailure(Throwable caught) {
				...
				if(caught instanceof TimeOutException){
					...
					Window.alert("FAIL: fetchResourceUseFor");// After FAIL ... write RPC Name  NOT Method Name..
				}
	...
	private void fetchDepartments() {
		rpcService.fetchStrategicDepartments(DTO.getStrategicId(),new AsyncCallback<ArrayList<StrategicDepartments>>(){
			...
			public void onSuccess(ArrayList<StrategicDepartments> strategicDepartments) {
				...
				for(int i=0; i< strategicDepartments.size(); i++){
					display.getRelevantDept().addItem(strategicDepartments.get(i).getDepartment().getDepartmentName(), strategicDepartments.get(i).getDepartment().getDepartmentId()+"");
					depIds.add(strategicDepartments.get(i).getDepartment().getDepartmentId());

				}
				selectedStrategicDepartments = strategicDepartments;
				fetchRelatedEmpAndSetSelected(depIds, strategicDepartments.get(strategicDepartments.size()-1).getResourcesIds());
			}});

	}

//	private void fetchRelatedEmpAndSetSelected(ArrayList<Integer> depIds, final ArrayList<Integer> resourcesSelectedForthisJob) {
//		rpcService.fetchEmployeesByDeptId( depIds, new AsyncCallback<ArrayList<Employee>>() {
//
//			@Override
...
//								String.valueOf(result.get(i).getEmployeeId()) +"" //relation id 0 for first time
//								);
//					}
//				}
//				
...
//					Window.alert("FAIL: fetchEmployeesByDeptId");// After FAIL ... write RPC Name  NOT Method Name..
//				}
//
//
//			}
			...
			public void onSuccess(ArrayList<Integer> resourcesSelectedForthisJob) {
				if( display.getProposedResources().getItemCount() >1){
					for(int j=0; j<resourcesSelectedForthisJob.size(); j++){
						for(int i=0; i<display.getProposedResources().getItemCount(); i++ ){
							if(Integer.parseInt(display.getProposedResources().getValue(i)) == resourcesSelectedForthisJob.get(j)){
			...
			public void onSuccess(ArrayList<Integer> softSkillsSelectedForthisJob) {
				if( display.getSoftSkill().getItemCount() >1){
					for(int j=0; j<softSkillsSelectedForthisJob.size(); j++){
						for(int i=0; i<display.getSoftSkill().getItemCount(); i++ ){
							if(Integer.parseInt(display.getSoftSkill().getValue(i)) == softSkillsSelectedForthisJob.get(j)){
