...
public class MySQLRdbHelper {
	...
	public User getAuthentication(String userid, String password) throws Exception
	{
		...
		try{
			...
			HibernateDetachUtility.nullOutUninitializedFields(users, HibernateDetachUtility.SerializationType.SERIALIZATION);
	...
	public ArrayList<Employee> fetchEmployees(int companyId) throws Exception {//Add this Throw Exception in all methods
	...
	public Employee fetchEmployeeById(int employeeId) throws Exception {//Add this Throw Exception in all methods
		...
		try{
			...
			for(Iterator it=rsList.iterator();it.hasNext();)
			{
				...
				HibernateDetachUtility.nullOutUninitializedFields(employee.getReportingTo(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				HibernateDetachUtility.nullOutUninitializedFields(employee.getReportingTo().getSkillId(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				HibernateDetachUtility.nullOutUninitializedFields(employee.getReportingTo().getRollId(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				HibernateDetachUtility.nullOutUninitializedFields(employee.getReportingTo().getReportingTo().getSkillId(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				HibernateDetachUtility.nullOutUninitializedFields(employee.getReportingTo().getReportingTo().getRollId(), HibernateDetachUtility.SerializationType.SERIALIZATION);
	...
	public String saveStrategic(Strategic strategic, User loggedInUser, HashMap<String, String> hm, int year, int companyId) {
	...
	public void saveOneStrategic(Strategic strategic, User loggedInUser, String todo, String tab, int year, int companyId){
		Session session = null;
		Transaction tr = null;
		try{
	...
	public void saveStrategicAudit(Strategic strategic){
		...
		try{
			...
			//			stragicAudit.setObjectiveOwner(strategic.getObjectiveOwner());// HERE Objective owner
	...
	private void approveStrategic(Strategic strategic, User loggedInUser,
			Strategic clientSideStrategic, Employee initiatedBy, Session session) throws Exception {
		...
		//		Employee approvedBy = (Employee) session.load(Employee.class, new Integer(loggedInUser.getEmployeeId().getEmployeeId()));
		//		Employee approvedBy = fetchEmployeeById(loggedInUser.getEmployeeId().getEmployeeId());
		//		strategic.setApprovedBy(approvedBy);
		strategic.setAuditableUnit(clientSideStrategic.getAuditableUnit());
		//		strategic.setObjectiveOwner(clientSideStrategic.getObjectiveOwner());// HERE Objective owner
	...
	private void submitStrategic(Strategic strategic, User loggedInUser, Strategic clientSideStrategic, Session session) {
	...
	private void initiateStrategic(Strategic strategic, User loggedInUser, Strategic clientSideStrategic, Session session) {
	...
	private void ammendStrategic(Strategic strategic, User loggedInUser, Strategic clientSideStrategic, Session session) {
	...
	public ArrayList<Strategic> fetchStrategic(HashMap<String, String> hm, int employeeId) {
		...
		try{
			...
			crit.add(Restrictions.eq("companyId", companyId));//    companyId check added
			crit.add(Restrictions.ne("status", "deleted"));
			...
			for(Iterator it=rsList.iterator();it.hasNext();)
			{
				...
				HibernateDetachUtility.nullOutUninitializedFields(strategic, HibernateDetachUtility.SerializationType.SERIALIZATION);
				HibernateDetachUtility.nullOutUninitializedFields(strategic.getInitiatedBy(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				HibernateDetachUtility.nullOutUninitializedFields(strategic.getInitiatedBy().getCountryId(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				HibernateDetachUtility.nullOutUninitializedFields(strategic.getInitiatedBy().getReportingTo(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				HibernateDetachUtility.nullOutUninitializedFields(strategic.getInitiatedBy().getReportingTo().getReportingTo(), HibernateDetachUtility.SerializationType.SERIALIZATION);

				HibernateDetachUtility.nullOutUninitializedFields(strategic.getInitiatedBy().getCityId(), HibernateDetachUtility.SerializationType.SERIALIZATION);

				HibernateDetachUtility.nullOutUninitializedFields(strategic.getAssignedTo(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				HibernateDetachUtility.nullOutUninitializedFields(strategic.getAssignedTo().getCountryId(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				HibernateDetachUtility.nullOutUninitializedFields(strategic.getAssignedTo().getReportingTo(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				HibernateDetachUtility.nullOutUninitializedFields(strategic.getAssignedTo().getCityId(), HibernateDetachUtility.SerializationType.SERIALIZATION);


				//				HibernateDetachUtility.nullOutUninitializedFields(strategic.getObjectiveOwner().getCityId(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				//				HibernateDetachUtility.nullOutUninitializedFields(strategic.getObjectiveOwner().getCountryId(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				//				HibernateDetachUtility.nullOutUninitializedFields(strategic.getObjectiveOwner().getReportingTo(), HibernateDetachUtility.SerializationType.SERIALIZATION);

				//Do not send those strategics which are Only SAVED and current loggedIn user is not the one who initiated that
				if(strategic.getPhase()> phaseValue){
					...
					if(strategic.getStatus().equalsIgnoreCase("saved") && strategic.getInitiatedBy().getEmployeeId()!= employeeId){
						...
						//Do not send those strategics which are Only SUBMITTED and current loggedIn user is not the one who initiated that or the to whoe this is assignd to 

					}else if(strategic.getStatus().equalsIgnoreCase("submitted") && ( strategic.getAssignedTo().getEmployeeId()!= employeeId && strategic.getInitiatedBy().getEmployeeId()!= employeeId)){
						//DO not send this strategy 
					}
					else if(strategic.getStatus().equalsIgnoreCase("initiated") && ( strategic.getAssignedTo().getEmployeeId()!= employeeId && strategic.getInitiatedBy().getEmployeeId()!= employeeId)){
						//DO not send this strategy 
					}
					else{
						strategics.add(strategic);
	...
	public ArrayList<StrategicDTO> fetchSchdulingStrategic(HashMap<String, String> hm, int year, int companyId) {
		...
		try{
			...
			for(Iterator it=rsList.iterator();it.hasNext();)
			{
				...
				//				strategicDTO.setDepartments(fetchStrategicDepartments(strategicDTO, strategic.getId()));
	...
	public ArrayList<RiskAssesmentDTO> fetchRiskAssesment(HashMap<String, String> hm, int employeeId, int year, int companyId) {
		...
		try{
			...
			for(Iterator it=rsList.iterator();it.hasNext();)
			{
				...
				HibernateDetachUtility.nullOutUninitializedFields(strategic.getInitiatedBy().getReportingTo(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				HibernateDetachUtility.nullOutUninitializedFields(strategic.getInitiatedBy().getCityId(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				//				HibernateDetachUtility.nullOutUninitializedFields(strategic.getObjectiveOwner(), HibernateDetachUtility.SerializationType.SERIALIZATION);// HERE Objective owner
				//				HibernateDetachUtility.nullOutUninitializedFields(strategic.getObjectiveOwner().getCountryId(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				HibernateDetachUtility.nullOutUninitializedFields(strategic.getInitiatedBy().getReportingTo().getReportingTo(), HibernateDetachUtility.SerializationType.SERIALIZATION);
	...
	public ArrayList<StrategicRisk> fetchRiskStrategic(int strategicId){
		...
		try{
			...
			for(Iterator it=rsList.iterator();it.hasNext();)
			{
				...
				//				HibernateDetachUtility.nullOutUninitializedFields(strategic.getAssignedTo().getCityId(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				//				HibernateDetachUtility.nullOutUninitializedFields(strategic.getAssignedTo().getReportingTo(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				//				HibernateDetachUtility.nullOutUninitializedFields(strategic.getAssignedTo().getReportingTo().getCountryId(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				//				HibernateDetachUtility.nullOutUninitializedFields(strategic.getAssignedTo().getReportingTo().getReportingTo(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				//				HibernateDetachUtility.nullOutUninitializedFields(strategic.getApprovedBy().getReportingTo(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				//				HibernateDetachUtility.nullOutUninitializedFields(strategic.getApprovedBy().getReportingTo().getReportingTo().getUserId(), HibernateDetachUtility.SerializationType.SERIALIZATION);
	...
	public ArrayList<DashBoardDTO> fetchDashBoard(User loggedInUser, int year, int companyId){
		...
		try{
			...
			ArrayList<Risk> risks = fetchEmployeeRisksForApproval(year, companyId, loggedInUser.getEmployeeId().getEmployeeId());;
			...
			ArrayList<AuditWork> auditWorks = fetchEmployeeAuditWorksForapproval(companyId, year, loggedInUser.getEmployeeId().getEmployeeId());
			...
			ArrayList<AuditStep> auditSteps = fetchEmployeeAuditStepsForApproval(year,companyId, loggedInUser.getEmployeeId().getEmployeeId());
			...
			ArrayList<Exceptions> exceptions = fetchEmployeeExceptionsForApproval(year, companyId, loggedInUser.getEmployeeId().getEmployeeId());
	...
	public boolean saveJobTimeEstimation(JobTimeEstimationDTO entity, ArrayList<SkillUpdateData> updateForSkills, int year, int companyId) {
	...
	public ArrayList<Employee> fetchEmployeesByDeptId(ArrayList<Integer> depIds, int companyId) {
		...
		try{
			...
			crit.createAlias("skillId", "skill");
			crit.createAlias("countryId", "countryId");
			crit.add(Restrictions.in("skill.skillId", depIds ) );//lets see what it 
			crit.add(Restrictions.ne("employeeId", 0));
			crit.add(Restrictions.eq("companyId", companyId));
			crit.createAlias("cityId", "city");
			//			crit.createAlias("companyId", "company");
			...
			for(Iterator it=rsList.iterator();it.hasNext();)
			{
				Employee employee =  (Employee)it.next();
				HibernateDetachUtility.nullOutUninitializedFields(employee, HibernateDetachUtility.SerializationType.SERIALIZATION);
				HibernateDetachUtility.nullOutUninitializedFields(employee.getReportingTo().getUserId(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				HibernateDetachUtility.nullOutUninitializedFields(employee.getReportingTo().getReportingTo(), HibernateDetachUtility.SerializationType.SERIALIZATION);
	...
	public ArrayList<Employee> fetchEmployeesBySkillId(int jobId, int companyId) {
		...
		try{
			...
			crit.createAlias("skillId", "skill");
			crit.createAlias("countryId", "countryId");
			crit.add(Restrictions.in("skill.skillId", skills ) );//lets see what it 
			crit.add(Restrictions.ne("employeeId", 0));
			crit.add(Restrictions.eq("companyId", companyId));
			crit.createAlias("cityId", "city");
			//			crit.createAlias("companyId", "company");
	...
	public boolean saveCreatedJob(JobCreationDTO job, int year, int companyId) {
		...
		try{
			...
			int jobCreationId = fetchJobCreationId(jobCreation.getJobId());// if job is aready created
	...
	public ArrayList<JobCreationDTO> fetchCreatedJobs( boolean getEmpRelation, boolean getSkillRelation, int year, int companyId ) {
		Session session = null;
		...
		try{
			...
			for(Iterator it=rsList.iterator();it.hasNext();)
			{
				...
				//				jobDTO.setDepartments(fetchStrategicDepartments(jobDTO, jobDTO.getJob().getJobCreationId()));
	...
	public ArrayList<JobEmployeeRelation> fetchEmployeeJobRelations(int jobCreationId) {
		...
		try{
			...
			for(Iterator it=rsList.iterator();it.hasNext();)
			{
				...
				HibernateDetachUtility.nullOutUninitializedFields(job.getEmployee().getReportingTo(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				HibernateDetachUtility.nullOutUninitializedFields(job.getEmployee().getReportingTo().getReportingTo(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				HibernateDetachUtility.nullOutUninitializedFields(job.getEmployee().getReportingTo().getReportingTo(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				HibernateDetachUtility.nullOutUninitializedFields(job.getEmployee().getReportingTo().getReportingTo(), HibernateDetachUtility.SerializationType.SERIALIZATION);
	...
	public JobCreation updateEndDateForJob(int jobId, String startDate, String endDate) throws Exception {
		...
		ArrayList<EmployeeJobDTO> employeeJobDTO = fetchEmployeeAvailabality(jobId,
				startDate, endDate);
	...
	private ArrayList<EmployeeJobDTO> fetchEmployeeAvailabality(int jobId,
		...
		for(int i=0; i< employeesOnThisJobs.size(); i++){
			ArrayList<Integer> jobOfThisEmployee = fetchjobEmployeeOtherThanGiveOne(employeesOnThisJobs.get(i).getEmployeeId(), jobId);
			for(int j=0; j<jobOfThisEmployee.size(); j++){
				...
				if(startDate!=null && job.getStartDate()!=null){
					...
					if((jobStart.equals(start)||jobStart.after(start)) && jobStart.before(end) || start.after(jobStart) && start.before(jobEnd)){
	...
	public ArrayList<AuditEngagement> fetchAllAuditEngagement(int loggedInEmployee, int year, int companyId  ) {
		...
		try{
			...
			for(Iterator it=rsList.iterator();it.hasNext();)		{			
				...
				HibernateDetachUtility.nullOutUninitializedFields(auditEngagement, HibernateDetachUtility.SerializationType.SERIALIZATION);
				HibernateDetachUtility.nullOutUninitializedFields(auditEngagement.getInitiatedBy().getReportingTo(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				HibernateDetachUtility.nullOutUninitializedFields(auditEngagement.getInitiatedBy().getReportingTo().getReportingTo(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				HibernateDetachUtility.nullOutUninitializedFields(auditEngagement.getApprovedBy(), HibernateDetachUtility.SerializationType.SERIALIZATION);
	...
	public ArrayList<Risk> fetchRisks(int auditEngId, int year, int companyId) {
		...
		try{
			...
			for(Iterator it=rsList.iterator();it.hasNext();)	{				
				...
				HibernateDetachUtility.nullOutUninitializedFields(risk, HibernateDetachUtility.SerializationType.SERIALIZATION);
				HibernateDetachUtility.nullOutUninitializedFields(risk.getInitiatedBy().getReportingTo(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				HibernateDetachUtility.nullOutUninitializedFields(risk.getInitiatedBy().getReportingTo().getReportingTo(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				HibernateDetachUtility.nullOutUninitializedFields(risk.getApprovedBy(), HibernateDetachUtility.SerializationType.SERIALIZATION);
	...
	private int fetchJobStatus(int jobId){
		...
		try{
			...
			for(int i=0; i< exceptions.size(); i++){
				...
				else if(exceptions.get(i).getFinalStatus()!=null && exceptions.get(i).getFinalStatus().equals("Approved")){
					status = 5;
	...
	public void saveAuditStepAndExceptions(AuditStep auditstep, ArrayList<Exceptions> exceptions, int year, int companyId) {
	...
	private void saveAuditStep(AuditStep auditstep, ArrayList<Exceptions> exceptions, int year, int companyId) {
		...
		try{
			...
			if(auditstep.getInitiatedBy() == null){
				auditstep.setInitiatedBy((Employee) session.get(Employee.class, auditstep.getInitiatedBy().getEmployeeId()));
			}
			if(auditstep.getApprovedBy() != null){
				auditstep.setApprovedBy((Employee) session.get(Employee.class, auditstep.getApprovedBy().getEmployeeId()));
			}
	...
	public AuditStep getSavedAuditStep(int jobid, int auditWorkId, int year, int companyId) {
		...
		try{
			...
			for(Iterator it=rsList.iterator();it.hasNext();)
			{
				...
				HibernateDetachUtility.nullOutUninitializedFields(auditStep.getInitiatedBy().getReportingTo(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				HibernateDetachUtility.nullOutUninitializedFields(auditStep.getInitiatedBy().getReportingTo().getReportingTo(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				HibernateDetachUtility.nullOutUninitializedFields(auditStep.getApprovedBy(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				HibernateDetachUtility.nullOutUninitializedFields(auditStep.getAuditWork().getApprovedBy(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				HibernateDetachUtility.nullOutUninitializedFields(auditStep.getAuditWork().getInitiatedBy().getReportingTo().getReportingTo(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				HibernateDetachUtility.nullOutUninitializedFields(auditStep.getAuditWork().getInitiatedBy().getReportingTo().getReportingTo(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				HibernateDetachUtility.nullOutUninitializedFields(auditStep.getAuditWork().getInitiatedBy(), HibernateDetachUtility.SerializationType.SERIALIZATION);
	...
	private void saveAuditWork(AuditWork auditWork) {
		...
		try{
			...
			if(auditWork.getInitiatedBy() == null){
				auditWork.setInitiatedBy((Employee) session.get(Employee.class, auditWork.getInitiatedBy().getEmployeeId()));
			}
			if(auditWork.getApprovedBy() != null){
				auditWork.setApprovedBy((Employee) session.get(Employee.class, auditWork.getApprovedBy().getEmployeeId()));
			}
	...
	public void updateKickoffStatus(int auditEngId, int year, int companyId, User loggedInUser) {
		...
		try{
			...
			if ( prevCreated != null )
			{
				...
				prevCreated.setInitiatedBy((Employee) session.get(Employee.class, loggedInUser.getEmployeeId().getEmployeeId()));
	...
	public ArrayList<AuditWork> fetchAuditWorkRows(int jocreationid, int companyId, int year) {
		...
		try{
			...
			for(Iterator it=rsList.iterator();it.hasNext();)
			{
				...
				HibernateDetachUtility.nullOutUninitializedFields(row.getInitiatedBy().getReportingTo(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				HibernateDetachUtility.nullOutUninitializedFields(row.getInitiatedBy().getReportingTo().getReportingTo(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				HibernateDetachUtility.nullOutUninitializedFields(row.getApprovedBy(), HibernateDetachUtility.SerializationType.SERIALIZATION);
	...
	public boolean isJobAuditStepApproved(int jobId){
		...
		try{
			...
			for(Iterator it=rsList.iterator();it.hasNext();)
			{
				...
				if(status == InternalAuditConstants.APPROVED && auditStep.getApprovedBy().getRollId().getRollId() == 1){
					jobAuditStepApproved = true;
				}else{
	...
	public String saveAuditNotification(int auditEngagementId, String message,
			String to, String cc, int year, int companyId) {
		Session session = null;
		try{
			session = sessionFactory.openSession();
	...
	public ArrayList<Strategic> fetchReportSearchResult( ArrayList<String> div, ArrayList<String> domain, ArrayList<String> risk, int year, int companyId) //int dept, int domain, String risk)
	{
		Session session = null;
	...
	public ArrayList<StrategicDepartments> fetchStrategicDepartmentsMutiple(
	...
	public ArrayList<Strategic> fetchReportAuditScheduling(
			ArrayList<String> div, ArrayList<String> domain,
		...
		try{
			...
			for(Iterator it=rsList.iterator();it.hasNext();){
				...
				HibernateDetachUtility.nullOutUninitializedFields(strategic.getAssignedTo().getReportingTo(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				HibernateDetachUtility.nullOutUninitializedFields(strategic.getAssignedTo().getCityId(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				//				HibernateDetachUtility.nullOutUninitializedFields(strategic.getObjectiveOwner().getCityId(), HibernateDetachUtility.SerializationType.SERIALIZATION); // HERE Objective owner
				//				HibernateDetachUtility.nullOutUninitializedFields(strategic.getObjectiveOwner().getCountryId(), HibernateDetachUtility.SerializationType.SERIALIZATION);
				//				HibernateDetachUtility.nullOutUninitializedFields(strategic.getObjectiveOwner().getReportingTo(), HibernateDetachUtility.SerializationType.SERIALIZATION);
		...
		if(userAvailable(employee.getEmail())){
			try{
				...
				if(employee.getRollId().getRollId() !=4 && employee.getRollId().getRollId() !=5){
					addAvailableHoursInSkills(employee.getSkillId().getSkillId(), employee.getTotalNumberOfHoursAvailable(), year, companyId);
	...
	public String approveSchedulingInJobTimeEstimation(int companyId)throws Exception {
		...
		try{
			...
			logger.warn(String.format("Exception occured in approveSchedulingInJobTimeEstimation", ex.getMessage()), ex);
			return null;
		}
		finally{
			session.close();
	...
	public ArrayList<JobCreation> fetchReportWithResourcesSearchResult(
			ArrayList<String> div, ArrayList<String> domain,
			ArrayList<String> risk, ArrayList<String> resources, int year,
			int companyId) {
		Session session = null;
		...
		try{
			...
			//				for(int i=0; i< resources.size(); i++){
			//					
			//					resourcesOR.add(Restrictions.eq("assigned.employeeId", Integer.parseInt(resources.get(i))));
			//				}
			//
	...
	public ArrayList<Integer> fetchJobsInExceptionReport(ArrayList<String> div, ArrayList<String> domain, ArrayList<String> risk, ArrayList<String> resources, ArrayList<String> jobs, int year, int companyId){
		Session session = null;
	...
	public ArrayList<Exceptions> fetchExceptionReports(ArrayList<String> div,
		...
		if(jobIds==null || jobIds.size()<1)return null;
			...
			if ( ! exceptionStatus.contains("All") )
			{
				for(int i=0; i< exceptionStatus.size(); i++){
					if(exceptionStatus.get(i).equals(InternalAuditConstants.APPROVEDBYIAHEAD)){
						...
						//						exceptionStatusOR.add(Restrictions.eq("isAgreed", true));
						//						exceptionStatusOR.add(Restrictions.eq("isImplemented", false));
						...
						//						exceptionStatusOR.add(Restrictions.lt("dueDate", new Date()));
						//						exceptionStatusOR.add(Restrictions.eq("isImplemented", false));
	...
	public String getExceptionStatus(Exceptions exception){
		...
		if(exception.getFinalStatus()!=null &&exception.getFinalStatus().equals("Approved")){
			...
			if(exception.getStatus()!=null &&exception.getStatus().equals("Approved")
			&& exception.getImplementaionComments()==null){
				exceptionStatus = InternalAuditConstants.COMMUNICATEDTOMGM;
			}else
				if(exception.getIsAgreed()==1 && exception.getIsImplemented()==0){
					exceptionStatus = InternalAuditConstants.AGREEDBYMGM;
				}else
					if(exception.getIsAgreed()==0 ){
						exceptionStatus = InternalAuditConstants.NOTAGREED;
					}else
						if(exception.getDueDate().before(new Date()) && exception.getIsImplemented()==0){
	...
	public String exportJobTimeAllocationReportToExcel(
			ArrayList<JobTimeAllocationReportDTO> excelDataList, String rootDir) {
		try {
	...
	public String exportExceptionsReportToExcel(
			ArrayList<ExceptionsReportDTO> excelDataList, String rootDir) {
		try {
	...
	public String exportAuditSchedulingReportToExcel(
			ArrayList<AuditSchedulingReportDTO> excelDataList, String rootDir) {
		try {
	...
	public ArrayList<String> fetchjobsInExecution(int year, int companyId)throws Exception {
		...
		try{
			...
			for(Iterator it=rsList.iterator();it.hasNext();)
			{
				...
				if(!isJobAuditStepApproved(job.getJobCreationId())){/////// FOR JOBS WHICH AUDIT STEP IS APPROVED
	...
	public ArrayList<String> fetchExceptionImplementationOverdue(int year,
	...
	public ArrayList<String> fetchExceptionImplementedAfterDueDate(int year,
			int companyId)throws Exception {
		Session session = null;
		ArrayList<String> exceptionImplementationsOverdueJobs = new ArrayList<String>();
		try{
	...
	public ArrayList<String> fetchExceptionImplemented(int year,
			int companyId)throws Exception {
		Session session = null;
		ArrayList<String> exceptionImplementationsOverdueJobs = new ArrayList<String>();
		try{
	...
	public ArrayList<String> fetchExceptioNotImplemented(int year,
			int companyId)throws Exception {
		Session session = null;
		ArrayList<String> exceptionImplementationsOverdueJobs = new ArrayList<String>();
		try{
	...
	public ArrayList<String> fetchExceptionImplementedWithindueDate(int year,
			int companyId)throws Exception {
		Session session = null;
		ArrayList<String> exceptionImplementationsOverdueJobs = new ArrayList<String>();
		try{
	...
	public ArrayList<String> fetchManagementCommentsOverdue(int year,
			int companyId)throws Exception {
		Session session = null;
...
//		ArrayList<String> implementedAfterDueDate = fetchExceptionImplementedAfterDueDate(year, companyId);
//		ArrayList<String> implementedWithinDueDate = fetchExceptionImplementedWithindueDate(year, companyId);
	...
	private void fetchReports(int year, int companyId,
			DashBoardNewDTO dashBoardDTO) {
	...
	public String updateUploadedAuditStepFile(Integer auditStepId,
			ArrayList<String> auditStepFiles)throws Exception {
